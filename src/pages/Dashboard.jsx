  ...
  return (
    <div>
      ...
      <div style={{ display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:12,marginBottom:24 }}>
        {[['Total Invoice','total',COLORS.primary,COLORS.primaryLight],['Invoice Valid','valid',COLORS.success,COLORS.successLight],['Invoice Tidak Valid','invalid',COLORS.danger,COLORS.dangerLight],['Draft','draft',COLORS.gray,COLORS.grayLight]].map(([lbl,key,c,bg])=>(
          <div key={key} style={{ ...styles.statCard, background:bg }}>
            <div style={{ fontSize:32,fontWeight:700,color:c }}>{stats[key]}</div>
      ...
      <div style={styles.card}>
        ...
        <table style={styles.table}>
          <thead><tr>{['No. Invoice','Penerima','Tanggal','Total','Status','Aksi'].map(h=><th key={h} style={styles.th}>{h}</th>)}</tr></thead>
          <tbody>
            {recent.map(inv=>(
