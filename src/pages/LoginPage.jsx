...
export default function LoginPage({ onLogin }) {
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:`linear-gradient(135deg, ${COLORS.primaryDark} 0%, ${COLORS.primary} 100%)`, padding: isMobile ? '16px' : '24px' }}>
      <div style={{ display:'flex', flexDirection: isMobile ? 'column' : 'row', background:'var(--color-background-primary)', borderRadius:16, overflow:'hidden', width: isMobile ? '100%' : 840, maxWidth:'100%', boxShadow:'0 24px 64px rgba(0,0,0,.2)' }}>
        
        {/* Left panel */}
        <div style={{ width: isMobile ? '100%' : 320, background:COLORS.primary, padding: isMobile ? '24px 20px' : 40, color:'#fff', display:'flex', flexDirection:'column', justifyContent:'center' }}>
          <div style={{ fontSize:22, fontWeight:700, marginBottom:6 }}>InvoiceSecure</div>
          <div style={{ fontSize:13, opacity:.8, marginBottom: isMobile ? 16 : 28 }}>Sistem Manajemen Invoice Elektronik</div>
          
          {/* Sembunyikan fitur list di mobile biar tidak terlalu panjang */}
          {!isMobile && ['Enkripsi data invoice dengan AES-256','Verifikasi integritas via SHA-256','Tanda tangan digital RSA-2048','QR Code untuk verifikasi publik','Tanpa instalasi software tambahan'].map((f,i) => (
          ...
          {/* Di mobile tampilkan versi ringkas */}
          {isMobile && (
          ...
          <div style={{ marginTop: isMobile ? 16 : 28, padding:'10px 14px', background:'rgba(255,255,255,.1)', borderRadius:10, fontSize:12, opacity:.9 }}>
            Demo: dimas@email.com / password123
          </div>
        </div>

        {/* Right panel */}
        <div style={{ flex:1, padding: isMobile ? '24px 20px' : 40 }}>
          ...
          {tab==='login' ? (
            <div>
              <h2 style={{ margin:'0 0 20px', fontSize: isMobile ? 18 : 20, fontWeight:500 }}>Masuk ke Akun Anda</h2>
              {['Alamat Email','Kata Sandi'].map((lbl,i) => (
                <div key={i} style={{ marginBottom:14 }}>
                  <label style={styles.label}>{lbl}</label>
                  <input style={{ ...styles.input, fontSize: isMobile ? 16 : 14 }} type={i===1?'password':'email'} placeholder={i===0?'contoh@email.com':'••••••••'}
            ...
            <div>
              <h2 style={{ margin:'0 0 20px', fontSize: isMobile ? 18 : 20, fontWeight:500 }}>Buat Akun Baru</h2>
              {[['Nama Lengkap','nama','text'],['Alamat Email','email','email'],['Kata Sandi','pass','password'],['Nama Perusahaan','perusahaan','text']].map(([lbl,key,type]) => (
                <div key={key} style={{ marginBottom:14 }}>
                  <label style={styles.label}>{lbl}</label>
                  <input style={{ ...styles.input, fontSize: isMobile ? 16 : 14 }} type={type} value={reg[key]} onChange={e=>setReg(r=>({...r,[key]:e.target.value}))} />
                </div>
              ))}
              {err && <div style={{ color:COLORS.danger, fontSize:13, marginBottom:12 }}>{err}</div>}
              <button onClick={handleRegister} style={{ ...styles.btn('primary'), width:'100%', justifyContent:'center', padding:'12px 0', fontSize:15 }}>Buat Akun</button>
            </div>
          )}
        </div>
      </div>
