...
export function AdminPengguna() {
  ...
  const toggleStatus = (id) => {
    ...
    if (u) { u.status = u.status==='aktif'?'nonaktif':'aktif'; setUsers([...DB.users]); }
    setToast({ msg:'Status pengguna diperbarui', type:'success' });
  };

  const filtered = users.filter(u=>!search||u.nama_lengkap.toLowerCase().includes(search.toLowerCase())||u.email.includes(search));
    ...
    <div>
      ...
      <div style={styles.card}>
        <table style={styles.table}>
          <thead><tr>{['ID','Nama Lengkap','Email','Perusahaan','Role','Status','Tgl. Daftar','Aksi'].map(h=><th key={h} style={styles.th}>{h}</th>)}</tr></thead>
          <tbody>
            {filtered.map(u=>(
              <tr key={u.user_id}>
        ...
        <div style={{ padding:'8px 0',fontSize:13,color:'var(--color-text-secondary)' }}>Menampilkan {filtered.length} dari {users.length} pengguna</div>
      </div>
        ...
        <Modal title="Tambah Pengguna Baru" onClose={()=>setShowAdd(false)}>
          ...
          <div style={{ display:'flex',gap:8,justifyContent:'flex-end' }}>
            ...
            <button onClick={()=>{
              ...
              DB.users.push(u); setUsers([...DB.users]); setShowAdd(false); setToast({msg:'Pengguna berhasil ditambahkan',type:'success'});
      ...
      <div style={styles.card}>
        ...
        {logs.length === 0 ? (
          <div style={{ textAlign:'center',padding:32,color:'var(--color-text-secondary)',fontSize:14 }}>
            Belum ada log verifikasi. Coba verifikasi beberapa invoice terlebih dahulu.
          </div>
        ) : (
          <table style={styles.table}>
            <thead><tr>{['Log ID','Invoice','IP Verifikator','Hash Digunakan','Hasil','Waktu'].map(h=><th key={h} style={styles.th}>{h}</th>)}</tr></thead>
            <tbody>
              {logs.map(log=>{
