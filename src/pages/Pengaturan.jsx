...
export default function Pengaturan({ user }) {
  ...
  return (
    <div>
      ...
      <div style={{ display:'flex',borderBottom:'1px solid var(--color-border-tertiary)',marginBottom:24 }}>
        {tabs.map(t=>(
          <button key={t} onClick={()=>setActiveTab(t)} style={{ padding:'10px 20px',background:'none',border:'none',borderBottom:activeTab===t?`2px solid ${COLORS.primary}`:'2px solid transparent',color:activeTab===t?COLORS.primary:'var(--color-text-secondary)',fontWeight:activeTab===t?500:400,cursor:'pointer',fontSize:14,textTransform:'capitalize',transition:'all .15s',marginBottom:-1 }}>
      ...
      {activeTab==='notifikasi' && (
        <div style={styles.card}>
          ...
          {[['Invoice berhasil dibuat',true],['Invoice berhasil diverifikasi pihak lain',true],['Invoice terdeteksi tidak valid',true],['Peringatan keamanan akun',true],['Ringkasan mingguan invoice',false]].map(([lbl,def])=>{
            ...
            return (
              <div key={lbl} style={{ display:'flex',alignItems:'center',justifyContent:'space-between',padding:'12px 0',borderBottom:'0.5px solid var(--color-border-tertiary)' }}>
                ...
                <div onClick={()=>setOn(!on)} style={{ width:40,height:22,borderRadius:11,background:on?COLORS.primary:'var(--color-border-secondary)',cursor:'pointer',position:'relative',transition:'background .2s' }}>
